...
int calculate(int o1,int o2,string op){
    if(op=="+") return o2+o1;
    else if(op=="-") return o2-o1;
    else if(op=="*") return o2*o1;
    else if(op=="/") return o2/o1;
    else if(op=="^") return (int)pow(o2,o1);
    else return -1;
}
int postfix_eval(Stack* &p,string postfix){
...
int main(){
    init(p,s1);
    string postfix;cout<<"Enter Postfix Numeric Expression:";
    getline(cin,postfix);
    int z=postfix_eval(p,postfix);
